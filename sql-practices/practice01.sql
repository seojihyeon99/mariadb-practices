...
select count(distinct(dept_no)) '매니저수'
from dept_manager;

-- 문제7.
-- 전체 부서를 출력하려고 합니다. 순서는 부서이름이 긴 순서대로 출력해 보세요.
select dept_name '부서명', length(dept_name) '길이'
from departments
order by '길이' desc;

-- 문제8.
-- 현재 급여가 120,000이상 받는 사원은 몇 명이나 있습니까?
select count(distinct(emp_no)) '사원수'
from salaries
where to_date = '9999-01-01' and salary >= 120000;

-- 문제9.
-- 어떤 직책들이 있나요? 중복 없이 이름이 긴 순서대로 출력해 보세요.
select distinct(title) '이름'
from titles
...
select title '직책', from_date '시작일', to_date '끝일'
from titles
where emp_no = 13250
